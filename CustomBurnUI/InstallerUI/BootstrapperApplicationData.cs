	...
	public class BootstrapperApplicationData
	{
...
#endif
                    manifestFile = Path.Combine(DefaultFolder, manifestFileName);
	            }
		...
		public static Package[] ParsePackagesFromXml(XPathNavigator root)
		{
			...
			foreach (XPathNavigator node in nodes)
			{
			    ...
			    string installCondition = GetAttribute(node, "InstallCondition");
			    {
			        if (installCondition != null)
			        {
			            package.InstallCondition = installCondition;
			            package.WillInstall = EvaluateCondition(package);
			        }
			    }

                string detectCondition = GetAttribute(node, "DetectCondition");
                {
                    if (detectCondition != null)
                    {
                        package.DetectCondition = detectCondition;
                        package.WillInstall = EvaluateCondition(package);
                    }
                }
	    ...
	    public static bool EvaluateCondition(Package package)
	    {
